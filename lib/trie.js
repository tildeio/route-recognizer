...
SegmentTrieNode.prototype = {
  ...
  */
  walk: function(path, params, regexPieces) {
    ...
    switch (this.type) {
      case "static":
        if (regexPieces) {
          ...
          segmentIndex = this.value.length + 1;
          nodeMatches = path.indexOf(this.value) === 0;
          if (nodeMatches) { path = path.substring(segmentIndex); }
        }
      break;
      case "param":
        if (regexPieces) {
      ...
      case "glob":
        ...
        params[this.value.substring(1)] = regexPieces.length;
    ...
    for (var i = 0; i < this.childNodes.length; i++) {
      nextNode = this.childNodes[i].walk(path, params, regexPieces);
...
RouteRecognizer.prototype = {
  ...
  recognize: function(path) {
    var params = {};
    var trieNode = this.rootState.walk(path, params);
    return trieNode;
  },
...
console.log(util.inspect(a.recognize('/posts/asdf/asdf'), false, null));
