...
function followEpsilons(states) {
  var followedStates = [];

  for (var i=0, l=states.length; i<l; i++) {
    var state = states[i];

    if (state instanceof EpsilonSegment) {
      followedStates = followedStates.concat(followEpsilons(state.nextStates));
    } else {
      followedStates.push(state);
    }
  }
