...
function parse(allSegments, route, names, specificity) {
  ...
  for (var i=0; i<segments.length; i++) {
    ...
    if (match = segment.match(/^:([^\/]+)$/)) {
      allSegments.push(new DynamicSegment(match[1]));
      names.push(match[1]);
      specificity += '3';
    } else if (match = segment.match(/^\*([^\/]+)$/)) {
      allSegments.push(new StarSegment(match[1]));
      specificity += '1';
      names.push(match[1]);
    } else if(segment === "") {
      allSegments.push(new EpsilonSegment());
      specificity += '2';
    } else {
      allSegments.push(new StaticSegment(segment));
      specificity += '4';
    }
  }
...
State.prototype = {
  regex: function () {
    if (!this._regex) {
      this._regex = buildRegex(this.pattern);
    }
    return this._regex;
  },
...
RouteRecognizer.prototype = {
  add: function RRADD(routes, options) {
    ...
    for (var i=0; i<routes.length; i++) {
      ...
      specificity = parse(allSegments, route.path, names, specificity);
