...
function State(charSpec, serialized) {
  if (serialized) {
    if (typeof serialized === "string") {
      serialized = JSON.parse(serialized);
    }

    this.charSpec = serialized.charSpec;
    this.nextStates = serialized.nextStates;
    this.charSpecs = serialized.charSpecs;
    this.regex = typeof serialized.regex !== "string" ? serialized.regex : new RegExp(serialized.regex);
    this.handlers = serialized.handlers;
    this.specificity = serialized.specificity;
  } else {
...
State.prototype = {
  ...
  match: function(ch) {
    ...
    for (var i=0; i<nextStates.length; i++) {
      if (nextStates[i] instanceof State) {
        child = nextStates[i];
      } else if (nextStates[i] === "__CIRCULAR__") {
        child = this;
      } else {
        child = new State(undefined, nextStates[i]);
      }
  ...
  serialize: function() {
    return JSON.stringify(this, function(key, value) {
      ...
      if (key === "regex") { return value.source; }
      if (key === "repeat" && !value) { return; }
      if (Array.isArray(value) && value.length === 0) { return; }
      if (typeof value === "object" && Object.keys(value).length === 0) { return; }

      if (value && value.nextStates && value.nextStates.length && value === value.nextStates[0]) {
        value.nextStates[0] = "__CIRCULAR__";
        return value;
      }
...
function recognizeChar(states, ch) {
  ...
  for (var i=0, l=states.length; i<l; i++) {
    ...
    if (states[i] instanceof State) {
      ...
      state = new State(undefined, states[i]);
    }
...
var RouteRecognizer = function(serialized) {
  this.rootState = new State(undefined, serialized);
  this.names = {};
};
...
RouteRecognizer.prototype = {
  ...
  toJSON: function() {
    return this.rootState.serialize();
  }
};
